import {
  ...
  SlidersHorizontal as ConfigureIcon,
} from "lucide-react";
...
export const IngestionSourceButton = ({
  ...
  useEffect(() => {
    ...
    const handleConfigChange = (e: Event) => {
      const detail = (
        e as CustomEvent<{
          appName?: string;
          config?: { streams?: Array<{ enabled?: boolean }> };
        }>
      ).detail;
      if (
        detail?.appName?.toLowerCase() === app.name.toLowerCase() &&
        detail?.config
      ) {
        const active = (detail.config.streams || []).filter((s) => s.enabled);
        setStreamSummary(
          `${active.length} stream${active.length === 1 ? "" : "s"} active`,
        );
      }
  ...
  }, [app.name, category, isSupport]);

  // Use optimistic state if set, otherwise fall back to actual
  const isEnabled =
    optimisticEnabled !== null ? optimisticEnabled : app.enabled;
  ...
  return (
    <Box
      ...
      data-tour={`ingestion-source-${app.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`}
    >
      <Tooltip
        title={
      ...
      >
        <IconButton
          ...
          sx={{
            ...
            transition:
              "transform 0.2s ease, opacity 0.15s ease, filter 0.15s ease, background-color 0.15s ease, border-color 0.15s ease",
            "&:hover": {
              transform: "scale(1.2)",
              borderColor: isBlocked
                ? "hsl(var(--destructive))"
                : highlighted
                  ...
                  : isEnabled
                    ? app.validated
                      ? "hsl(var(--severity-low) / 0.30)"
                      : "hsl(var(--severity-medium) / 0.50)"
                    : "transparent",
              bgcolor: isBlocked
                ...
                : highlighted
                  ...
                  : isEnabled
                    ? app.validated
                      ? "hsl(var(--severity-low) / 0.18)"
                      : "hsl(var(--severity-medium) / 0.22)"
      ...
      >
        {isBlocked && (
          ...
          >
            ...
            >
              {health?.primaryProblem?.description ||
                "The runtime location used by this workflow is offline."}
            </Typography>
            <Button
              size="small"
              variant="outlined"
              href={
                health?.primaryProblem?.actionUrl || "/admin/runtime-locations"
              }
              sx={{
                fontSize: "0.7rem",
                py: 0.25,
        ...
        >
          ...
          {isBlocked ? (
            ...
            <Chip
              label={variant === "forward" ? "Not Forwarding" : "Not Ingesting"}
              size="small"
              sx={{
                ml: 0.5,
                height: 18,
          ...
          {category === "vulnerabilities" && isSupport && (
            ...
            >
              ...
              >
                ...
                <Tooltip
                  title="This configuration is restricted to Shuffle support users"
                  arrow
                >
                  <Chip
                    label="Support only"
